...
BINDIR="${BINDIR:-./bin}"
AGE_USER_REPO="${AGE_USER_REPO:-FiloSottile/age}"
TAGARG=latest
LOG_LEVEL=2

GITHUB_DOWNLOAD="https://github.com/${AGE_USER_REPO}/releases/download"
...
main() {
	...
	*) arch="${GOARCH}" ;;
	...
	# download tarball
	NAME="age-v${VERSION}-${GOOS}${GOOS_EXTRA}-${arch}"
	TARBALL="${NAME}.${FORMAT}"
	...
	BINARY="age${BINSUFFIX}"
	install -- "${tmpdir}/age/${BINARY}" "${BINDIR}/"
...
real_tag() {
	...
	release_url="https://github.com/${AGE_USER_REPO}/releases/${tag}"
